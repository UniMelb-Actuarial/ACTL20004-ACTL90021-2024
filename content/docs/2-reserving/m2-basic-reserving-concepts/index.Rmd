...
output_name <- "24-Top-M2-lec" # change latest slides below, too!
...
[^1]: References: Chapter 1 of @Tay00 | $\; \rightarrow$ [\textcolor{blue}{\underline{latest slides}}](https://gim-am3.netlify.app/output/24-Top-M2-lec.pdf) 
...
Note that figures, notation, examples and structure are mostly drawn from @Tay00, which is the main reference for Modules 2--5.
...
\begin{eqnarray*}
\overline{P}(k) &=& \sum_{i=0}^I \sum_{m=k-i+1}^\infty C(i,m) \left[1+e(i,m)\right]d(i+m) \\
&=& \sum_{i=0}^I \sum_{m=k-i+1}^\infty C^*(i,m) \frac{\lambda(i+m)}{\lambda_0}\left[1+e(i,m)\right]d(i+m).
\end{eqnarray*}
