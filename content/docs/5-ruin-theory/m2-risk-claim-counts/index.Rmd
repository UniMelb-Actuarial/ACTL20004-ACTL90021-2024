...
#
graphics_folder <- "module-1"
output_name <- "23-Top-M1-lec" # change latest slides below, too!
...
[^1]: References: Chapter 7.1-7.3 and 8.1-8.3 of @Jos13 | $\; \rightarrow$ [\textcolor{blue}{\underline{latest slides}}](https://gim-am3.netlify.app/output/23-Top-M1-lec.pdf) 
...
- ($\adv$) approximate with the help of the moments of $S$ (Module 4)
...
- discrete case:  @BoGeHiJoNe97 Example 2.3.1 on page 35
- continuous case:  @BoGeHiJoNe97 Example 2.3.2 on page 36
...
$$\begin{array}{rcll}
f_{1} \left( y \right)  & = & \frac{1}{4}, \frac{1}{2},\frac{1}{4} & \text{ for } y=0,1,2 \\
f_{2} \left( y \right)  &=& \frac{1}{2}, \frac{1}{2} & \text{ for } y=0,2 \\
f_{3} \left( y \right)  &=& \frac{1}{4}, \frac{1}{2},\frac{1}{4} & \text{ for } y=0,2,4
\end{array}$$
...
$$\begin{array}{rcl}
f_{1+2}(2) &\!=& \!\! 1/4 \cdot 1/2+1/2 \cdot 0+1/4 \cdot 1/2 \\
f_{1+2+3}(4) &\!=& \!\! 1/8 \cdot 1/4+2/8 \cdot 0+2/8 \cdot 1/2+2/8 \cdot 0+1/8 \cdot 1/4
\end{array}$$
...
- Sometimes, $M_S(t)$ or $p_S(t)$ can be recognised: this is the case for infinitely divisible distributions (Normal, Poisson, Inverse Gaussian, \ldots) and certain other distributions (Binomial, Negative binomial).
...
Var(S) & = & E\left[ Var(S|N) \right] + Var\left( E[S|N] \right) \\
  & = & E\left[ N Var(Y) \right] + Var(E[Y] N) \\
  & = & E[N] Var(Y) + E[Y]^2 Var(N) \\
  & = & E[N] (E[Y^2] - E[Y]^2)+ E[Y]^2 Var(N) \\
  & = &  E[N]E[Y^2] + E[Y]^2 \left( Var(N)-E[N] \right).
\end{array}$$
...
$$\begin{array}{rcl}
M_S(t) &=& E\left[e^{tS}\right]= E\left[ E\left[ \left.e^{t(Y_1+Y_2+\ldots+Y_N)}\right|N \right]\right] \\
  &=&   E\left[ M_Y(t)^N\right]=E\left[ e^{N \ln M_Y(t)} \right]  \\
  &=&    M_N\left(\ln M_Y(t)\right)
\end{array}$$

### Example (@BoGeHiJoNe97, 12.2.1)
...
### Example, continued (@BoGeHiJoNe97, 12.2.3)
...
$$f_S(s)=\begin{cases}
p=\Pr[N=0] \mbox{ (probability mass)}&s=0;\\
(1-p)(pe^{-ps}) \mbox{ (probability density)}&s>0.
\end{cases}$$
...
We will make extensive use of the function `aggregateDist` from the package `actuar` [@DiGoPi08]:
...
This is input as a vector of claim amount probability _masses_ after the argument `model.sev=`. The first element must be $\Pr[Y=0]$.
...
```{r aggrConv2,fig.height=5.4,collapse=TRUE}
summary(Fs)
plot(Fs)
```
...
- It makes no sense to talk about frequency in an insurance portfolio without considering **exposure**. Chapter 4 of @WeMo10 defines *exposure* as "the basic unit that measures a policy's exposure to loss".
...
- @Wut20 calls exposure "volume", denoted $v$, and defines the **claims frequency** as
$$ \frac{N}{v}.$$
...
- A summary table of those distributions is also given in @BoGeHiJoNe97, see Table 12.3.1 on page 376.
...
\Pr[N=n]&=&\int_0^\infty \Pr[N=n|\Lambda=\lambda]  d H(\lambda)=\int_0^\infty \frac{e^{-\lambda v}(\lambda v)^n}{n!} d H(\lambda); \\ \\
E[N] &=& E\left[E[N|\Lambda]\right] = E[\Lambda]v=\lambda v; \\ \\
Var(N) &=& E\left[Var(N|\Lambda)\right] + Var\left(E[N|\Lambda]\right)= \lambda v+v^2 Var(\Lambda) > \lambda v; \\ \\
M_N(t) &=&E\left[e^{tN}\right] = E\left[E\left[e^{tN}|\Lambda\right]\right]=E\left[e^{\Lambda v (e^t-1)}\right]=M_\Lambda(v[e^t-1]).
\end{array}$$
...
$$\begin{array}{rcl}
M_N(t) &=& E[e^{tN}]=E\left[  E[e^{tN} | \Theta] \right] = E\left[ e^{\Theta \lambda v (e^t-1)} \right] \\\\ \displaystyle
&=& \left( \frac{\gamma}{\gamma-\lambda v (e^t-1)}  \right)^\gamma  = \left( \frac{\gamma}{\gamma +\lambda v -\lambda v e^t}  \right)^\gamma  = \left( \frac{\frac{\gamma}{\lambda v +\gamma}}{1 -\frac{\lambda v}{\lambda v +\gamma} e^t}  \right)^\gamma,
\end{array}$$
...
$$\begin{array}{rcl}
E[N] &=& \lambda v, \\
Var(N) &=& \lambda v \left(1+\frac{\lambda v}{\gamma}\right) > \lambda v,\\
\text{Vco}\left( \frac{N}{v} \right) &=& \sqrt{(\lambda v)^{-1} + \gamma^{-1}}.
\end{array}$$
...
- "Bottom-up" modelling
- In @BoGeHiJoNe97, this is Theorem 12.4.1.
...
### Example 12.4.1 of @BoGeHiJoNe97
...
$$\begin{array}{rcl}
A_k \cap A_l &=& \emptyset \text{ for all }k\neq l\quad \text{(no overlap)}; \\
\cup_{k=1}^n A_k &=& \mathbb{R}_+ \times \{1,\ldots,m\} \quad \text{(all-inclusive)}.
\end{array}$$
...
Proof: see tutorial exercise `los18`. Note also that this is a special case of Theorem 2.14, and is Theorem 12.4.2 of @BoGeHiJoNe97.
...
[@BoGeHiJoNe97, Example 12.4.2] Suppose $S$ has a compound Poisson distribution with $\lambda =0.8$ and individual claim amount distribution
...
$$\begin{array}{rcl}
S_{\text{sc}} &=& \sum_{i=1}^N Y_i 1_{\{Y_i\le M\}}, \text{ and}\\
S_{\text{lc}} &=& \sum_{i=1}^N Y_i 1_{\{Y_i > M\}},
\end{array}$$
respectively.

---

-  Theorem 2.14 implies that $S_{\text{sc}}$ and $S_{\text{lc}}$ are \textit{independent} and compound Poisson distributed with
$$\begin{array}{rcl}
S_{\text{sc}} &\sim & \text{CompPoi}(\lambda_{\text{sc}}v = \lambda G(M)v, \\
&&\quad\quad\quad\quad G_{\text{sc}}(y)=\Pr[Y_1\le y|Y_1\le M]), \text{ and} \\
S_{\text{lc}} &\sim & \text{CompPoi}(\lambda_{\text{lc}}v = \lambda (1-G(M))v, \\
&&\quad\quad\quad\quad G_{\text{lc}}(y)=\Pr[Y_1\le y|Y_1> M]),
\end{array}$$
respectively.
- The distribution of
$$S=S_{\text{sc}} + S_{\text{lc}}$$
can then be obtained by a simple convolution of distributions of  $S_{\text{sc}}$ and $S_{\text{lc}}$ (thanks to independence); see Module 4 for examples (\adv).
...
This is the $(a,b)$ class of "Panjer distributions". This means that $\Pr[N=n]$ can be obtained recursively with initial value $\Pr[N=0]$; see @Wut20, Definition 4.6.

The exhaustive list of its members [see @Wut20 Lemma 4.7] is
...
$$p_k^T = \left\{ \begin{array}{ll}
0, & k=0; \\
\frac{p_k}{1-p_0}, & k=1,2,\ldots.
\end{array}\right.,$$
...
$$p_k^M = \left( 1-\frac{1-p_0^M}{1-p_0}\right)1_{\{k=0\}} +
\frac{1-p_0^M}{1-p_0}p_k.$$
Alternatively,
$$p_k^M = \left\{ \begin{array}{ll}
p_0^M, & k=0; \\
\frac{1-p_0^M}{1-p_0}p_k, & k=1,2,\ldots.
\end{array}\right.,$$
...
```{r zero,fig.height=5.8}
plot(dpois(0:7,2.5),pch=20,col="red",ylim=c(0,0.3),cex=1.5,type="b")
points(dztpois(0:7,2.5),pch=20,col="blue",type="b")
points(dzmpois(0:7,2.5,2*dpois(0,2.5)),pch=20,col="green",type="b")
```
