...
output_name <- "24-Top-M1-lec" # change latest slides below, too!
...
[^1]: References: Chapter 7.1-7.3 and 8.1-8.3 of @Jos13 | $\; \rightarrow$ [\textcolor{blue}{\underline{latest slides}}](https://topics-actl.netlify.app/output/24-Top-M1-lec.pdf) 
...
```{r 1stdom,echo=F,fig.height=7}
par(cex=1.4)
curve(pnorm(x,mean=1,sd=1),from=-2, to=5,lwd=2,col="red",xlab="x",ylab="F(x)")
curve(pnorm(x,mean=2,sd=1),from=-2, to=5,lwd=2,col="blue",add=T)
text(1,0.7,"B is normal(1,1)",col="red",adj=1)
text(2,0.3,"A is normal(2,1)",col="blue",adj=0)
```
...
```{r 2stdom2,echo=F,fig.height=7}
par(cex=1.4)
integralB <- c()
integralA <- c()
for (i in c(-3+1:8000/1000)){
    integralB <- c(integralB,integrate(pnorm,lower=-Inf,upper=i,mean=1,sd=2)[1]$value)
    integralA <- c(integralA,integrate(pnorm,lower=-Inf,upper=i,mean=1,sd=1)[1]$value)
}
plot(c(-3+1:8000/1000),integralB,lwd=2,col="red",xlab="x",ylab="Integral of F(y) over -Inf to x",type="l")
lines(c(-3+1:8000/1000),integralA,lwd=2,col="blue")
text(.2,0.7,"B is normal(1,2)",col="red",adj=1)
text(1.3,0.3,"A is normal(1,1)",col="blue",adj=0)
```
...
```{r 2stdom,echo=F,fig.height=7}
par(cex=1.4)
curve(pnorm(x,mean=1,sd=2),from=-3, to=5,lwd=2,col="red",xlab="x",ylab="F(x)")
curve(pnorm(x,mean=1,sd=1),from=-3, to=5,lwd=2,col="blue",add=T)
text(2.2,0.6,"B is normal(1,2)",col="red",adj=0)
text(1.2,0.7,"A is normal(1,1)",col="blue",adj=1)
```
...
include_graphics(sprintf("%s/comparison.png", graphics_path), error = F)
```
...
```{r umbicecreamgamma0,fig.show='hide'} 
power <- function(x,gamma){ (x^gamma-1)/gamma }

umbrella <- function(gamma,w0){
    expectedutility <- (power(w0+30000,gamma)+2*power(w0+4000,gamma)+power(w0+2000,gamma))/4
    return( (expectedutility*gamma+1)^(1/gamma) ) }

icecream <- function(gamma,w0){
    expectedutility <- (power(w0+2000,gamma)+2*power(w0+8000,gamma)+power(w0+16000,gamma))/4
    return( (expectedutility*gamma+1)^(1/gamma) ) }
```
...
```{r umbicecreamgamma,fig.show='hide'} 
par(cex=1.4)
curve(umbrella(x,0),from=0.01,to=0.99,lwd=2,col="blue",xlab="Parameter gamma (the smaller the more risk averse)",ylab="Certainty equivalent")
curve(icecream(x,0),from=0.01,to=0.99,add=TRUE,lwd=2,col="red")
abline(v=0.5849920142672,col="green")
text(0.6,6000,paste(signif(0.5849920142672,digits=3),sep=""),col="green",adj=0)
...
```{r umbicecreamgamma,echo=F,results='hide',fig.height=7}
```
...
```{r umbicecreamgamma2,fig.show='hide'} 
par(cex=1.4)
curve(umbrella(x,100000)-icecream(x,100000),from=0.01,to=0.99,lwd=2,col="black",xlab="Parameter gamma (the smaller the more risk averse)",ylab="Additional certainty equivalent of umbrellas")
abline(v=0.5849920142672,col="green")
```
...
```{r umbicecreamgamma2,echo=F,results='hide',fig.height=7}
```
...
1. Gather data (choices) about an individual, and try to "fit" an appropriate utility function to those choices.
